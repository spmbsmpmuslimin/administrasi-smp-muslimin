// src/components/DashboardAdmin.js

import { useState } from "react";
import { Settings, BarChart3, FileText, Users } from "lucide-react";
import RaportConfig from "./RaportConfig";

function DashboardAdmin({ user, onShowToast, darkMode }) {
  const [activeTab, setActiveTab] = useState("monitoring");

  // Cek apakah user adalah admin
  if (user?.role !== "admin") {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-red-600 mb-2">
            Akses Ditolak
          </h2>
          <p className="text-theme-secondary">Halaman ini hanya untuk Administrator</p>
        </div>
      </div>
    );
  }

  return (
    <div className={`min-h-screen ${darkMode ? "bg-theme-bg" : "bg-theme-surface"}`}>
      {/* Header */}
      <div className={`${darkMode ? "bg-gray-800" : "bg-theme-bg"} shadow-md`}>
        <div className="max-w-7xl mx-auto px-4 py-4">
          <h1
            className={`text-2xl font-bold ${
              darkMode ? "text-white" : "text-theme"
            }`}>
            Admin Dashboard E-Raport
          </h1>
          <p
            className={`text-sm ${
              darkMode ? "text-gray-400" : "text-theme-secondary"
            }`}>
            Kelola sistem e-raport sekolah
          </p>
        </div>
      </div>

      {/* Tab Navigation */}
      <div className={`${darkMode ? "bg-gray-800" : "bg-theme-bg"} border-b`}>
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex gap-2 overflow-x-auto">
            <button
              onClick={() => setActiveTab("monitoring")}
              className={`flex items-center gap-2 px-4 py-3 font-medium border-b-2 transition-colors ${
                activeTab === "monitoring"
                  ? "border-blue-600 text-blue-600"
                  : `border-transparent ${
                      darkMode
                        ? "text-gray-400 hover:text-gray-200"
                        : "text-theme-secondary hover:text-theme"
                    }`
              }`}>
              <BarChart3 size={20} />
              Monitoring
            </button>

            <button
              onClick={() => setActiveTab("tp")}
              className={`flex items-center gap-2 px-4 py-3 font-medium border-b-2 transition-colors ${
                activeTab === "tp"
                  ? "border-blue-600 text-blue-600"
                  : `border-transparent ${
                      darkMode
                        ? "text-gray-400 hover:text-gray-200"
                        : "text-theme-secondary hover:text-theme"
                    }`
              }`}>
              <FileText size={20} />
              Tujuan Pembelajaran
            </button>

            <button
              onClick={() => setActiveTab("users")}
              className={`flex items-center gap-2 px-4 py-3 font-medium border-b-2 transition-colors ${
                activeTab === "users"
                  ? "border-blue-600 text-blue-600"
                  : `border-transparent ${
                      darkMode
                        ? "text-gray-400 hover:text-gray-200"
                        : "text-theme-secondary hover:text-theme"
                    }`
              }`}>
              <Users size={20} />
              Manajemen User
            </button>

            <button
              onClick={() => setActiveTab("raport")}
              className={`flex items-center gap-2 px-4 py-3 font-medium border-b-2 transition-colors ${
                activeTab === "raport"
                  ? "border-blue-600 text-blue-600"
                  : `border-transparent ${
                      darkMode
                        ? "text-gray-400 hover:text-gray-200"
                        : "text-theme-secondary hover:text-theme"
                    }`
              }`}>
              <Settings size={20} />
              Konfigurasi E-Raport
            </button>
          </div>
        </div>
      </div>

      {/* Content Area */}
      <div className="max-w-7xl mx-auto">
        {activeTab === "monitoring" && (
          <div className="p-6">
            <div
              className={`${
                darkMode ? "bg-gray-800" : "bg-theme-bg"
              } rounded-lg shadow p-6`}>
              <h2
                className={`text-xl font-bold mb-4 ${
                  darkMode ? "text-white" : "text-theme"
                }`}>
                Monitoring (Coming Soon)
              </h2>
              <p className={darkMode ? "text-gray-400" : "text-theme-secondary"}>
                Fitur monitoring akan segera hadir...
              </p>
            </div>
          </div>
        )}

        {activeTab === "raport" && (
          <RaportConfig
            user={user}
            showToast={onShowToast}
            darkMode={darkMode}
          />
        )}

        {activeTab === "tp" && (
          <div className="p-6">
            <div
              className={`${
                darkMode ? "bg-gray-800" : "bg-theme-bg"
              } rounded-lg shadow p-6`}>
              <h2
                className={`text-xl font-bold mb-4 ${
                  darkMode ? "text-white" : "text-theme"
                }`}>
                Tujuan Pembelajaran (Coming Soon)
              </h2>
              <p className={darkMode ? "text-gray-400" : "text-theme-secondary"}>
                Fitur TP akan segera hadir...
              </p>
            </div>
          </div>
        )}

        {activeTab === "users" && (
          <div className="p-6">
            <div
              className={`${
                darkMode ? "bg-gray-800" : "bg-theme-bg"
              } rounded-lg shadow p-6`}>
              <h2
                className={`text-xl font-bold mb-4 ${
                  darkMode ? "text-white" : "text-theme"
                }`}>
                Manajemen User (Coming Soon)
              </h2>
              <p className={darkMode ? "text-gray-400" : "text-theme-secondary"}>
                Fitur user management akan segera hadir...
              </p>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}

export default DashboardAdmin;
