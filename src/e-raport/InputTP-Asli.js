...
function InputTP({ user, onShowToast, darkMode }) {
  ...
  const loadAcademicInfoAndAssignments = async () => {
    try {
      ...
      if (assignmentsError) {
        console.error("❌ Query error:", assignmentsError);
  ...
  const loadTP = async () => {
    try {
      if (!selectedSemesterId) {
        console.error("Semester ID belum dipilih");
        return;
      }
  ...
  const handleImportExcel = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase.from("tujuan_pembelajaran").insert(imported);

      if (error) throw error;

      if (onShowToast)
        onShowToast(`Import berhasil! ${imported.length} data TP ditambahkan.`, "success");
      loadTP();
    } catch (error) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      worksheet.getCell("A4").value = `Semester: ${semesterText}`;
  ...
  const handleAddRow = async () => {
    try {
      ...
      if (onShowToast) onShowToast(`Gagal menambah baris: ${error.message}`, "error");
    }
  };
  ...
  const handleSave = async () => {
    try {
      if (!selectedSemesterId) {
        if (onShowToast) onShowToast("Semester tidak tersedia", "error");
        return;
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (onShowToast) onShowToast(`Gagal menghapus: ${error.message}`, "error");
    }
  };
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      ...
      loadTP();
      if (onShowToast) {
        onShowToast(`TP ${!currentStatus ? "diaktifkan" : "dinonaktifkan"}`, "success");
      }
    } catch (error) {
          ...
          {!selectedMapel || !tingkat || !selectedSemesterId ? (
            ...
            >
              ...
              {academicInfo && (
                ...
                >
                  <span className="text-sm font-medium">Tahun Ajaran: {academicInfo.year}</span>
                </div>
              )}
            </div>
          ) : (
                        ...
                        <tr>
                          <td
                            ...
                            className={`text-center py-8 sm:py-12 ${
                              darkMode ? "text-gray-400 bg-gray-800/50" : "text-gray-500 bg-blue-50"
                            }`}
                          >
                            <div className="flex flex-col items-center">
                              ...
                              <p className="text-sm">Tambah data atau import dari Excel</p>
                              <div
                              ...
                              >
                                Semester:{" "}
                                {(() => {
                                  const sem = availableSemesters.find(
                          ...
                          >
                            {editingId === tp.id ? (
                              ...
                              <>
                                ...
                                >
                                  <button
                                    onClick={() => handleToggleStatus(tp.id, tp.is_active)}
                                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                                      tp.is_active ? "bg-green-500" : "bg-gray-300"
                                    }`}
                                  >
                                    <span
                                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                                        tp.is_active ? "translate-x-6" : "translate-x-1"
                                      }`}
                                    />
                                  </button>
                                </td>
