import React from "react";
import { Construction } from "lucide-react";

function InputEkstrakurikuler({ darkMode }) {
  return (
    <div
      className={`min-h-screen flex items-center justify-center p-4 ${
        darkMode ? "bg-theme-bg" : "bg-gradient-to-br from-blue-50 to-sky-100"
      }`}>
      <div
        className={`max-w-md w-full rounded-2xl shadow-xl p-8 text-center ${
          darkMode
            ? "bg-gray-800 border border-theme"
            : "bg-theme-bg border border-blue-200"
        }`}>
        {/* Icon */}
        <div
          className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${
            darkMode ? "bg-purple-900/30" : "bg-purple-100"
          }`}>
          <Construction
            size={40}
            className={darkMode ? "text-purple-400" : "text-purple-600"}
          />
        </div>

        {/* Title */}
        <h2
          className={`text-2xl font-bold mb-3 ${
            darkMode ? "text-white" : "text-theme"
          }`}>
          Input Ekstrakurikuler
        </h2>

        {/* Message */}
        <p
          className={`text-base mb-6 ${
            darkMode ? "text-gray-300" : "text-theme-secondary"
          }`}>
          Halaman ini masih dalam pengembangan
        </p>

        {/* Badge */}
        <div
          className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium ${
            darkMode
              ? "bg-yellow-900/30 text-yellow-300 border border-yellow-700"
              : "bg-yellow-100 text-yellow-800 border border-yellow-300"
          }`}>
          <span className="animate-pulse">🚧</span>
          Coming Soon
        </div>

        {/* Additional Info */}
        <p
          className={`mt-6 text-xs ${
            darkMode ? "text-theme-secondary" : "text-gray-400"
          }`}>
          Fitur input nilai ekstrakurikuler akan segera hadir
        </p>
      </div>
    </div>
  );
}

export default InputEkstrakurikuler;
