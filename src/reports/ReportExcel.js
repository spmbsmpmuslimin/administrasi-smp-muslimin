...
const exportToExcel = async (data, headers, metadata, options = {}) => {
  const { role = "admin", reportType = "general", styling = "default" } = options;
...
const addBKSpecialSection = async (worksheet, startRow, data) => {
  ...
  urgencyRow.getCell(
    2
  ).value = `Darurat: ${urgencyCount.darurat} | Tinggi: ${urgencyCount.tinggi} | Sedang: ${urgencyCount.sedang} | Rendah: ${urgencyCount.rendah}`;
...
const addFooter = async (worksheet, startRow, role) => {
  ...
  footerRow.getCell(1).value = `Generated by SMP Muslimin Cililin - ${new Date().toLocaleString(
    "id-ID",
    { timeZone: "Asia/Jakarta" }
  )}`;
