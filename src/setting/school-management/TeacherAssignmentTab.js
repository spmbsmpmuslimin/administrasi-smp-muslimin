...
const TeacherAssignmentTab = ({ user, showToast, schoolConfig }) => {
  ...
  // ✅ NEW: assignment yang guru-nya udah gak ada jadwal (teacher_schedules)
  // sama sekali di kelas itu -- kemungkinan jadwal berubah di tengah jalan
  // tapi teacher_assignments-nya kelupaan di-update manual. BP/BK selalu
  // dikecualiin (guru BK emang gak pernah punya jam masuk kelas reguler).
  const [staleAssignments, setStaleAssignments] = useState([]);
  const [staleBannerExpanded, setStaleBannerExpanded] = useState(false);
  ...
  // ✅ TAMBAH: cek histori jurnal_harian yang nempel ke assignment sebelum dihapus
  const [jurnalCount, setJurnalCount] = useState(0); // -1 = gagal cek, anggap ada (fail-safe)
  const [checkingJurnal, setCheckingJurnal] = useState(false);
  const [confirmRiskAck, setConfirmRiskAck] = useState(false);
  ...
  const loadAssignments = async () => {
    try {
      ...
      const { activeSemester, activeSemesterId: activeYearId } =
      ...
      if (assignmentsData && assignmentsData.length > 0) {
        ...
        checkStaleAssignments(filteredData);
      } else {
        setAssignments([]);
        calculateStats([]);
        setStaleAssignments([]);
  ...
  // ✅ NEW: cek assignment yang guru-nya udah gak ada jadwal sama sekali
  // di kelas itu (kemungkinan jadwal berubah tapi teacher_assignments
  // kelupaan di-update). Dibandingin ke teacher_schedules (bukan ke file
  // Excel kayak syncPreview di AdminJadwalMassal.js, biar bisa jalan
  // kapan aja tanpa perlu upload apa-apa).
  //
  // ⚠️ PENTING: teacher_assignments.teacher_id nyimpen KODE GURU (mis.
  // "G-01"), sedangkan teacher_schedules.teacher_id nyimpen UUID
  // (users.id). Dua-duanya HARUS diterjemahin dulu lewat users.teacher_id
  // (kode) -> users.id (UUID) sebelum dibandingin -- kalau ngebandingin
  // langsung, semua assignment bakal keflag stale (kode != UUID, dua
  // format yang beda total). Fetch `users` sendiri di sini (bukan numpang
  // state `teachers`) biar gak kena stale-closure -- state `teachers`
  // dari setTeachers() belum tentu ke-update duluan pas checkStaleAssignments
  // ini dipanggil dari alur loadAllData() awal.
  const checkStaleAssignments = async (assignmentData) => {
    setCheckingStale(true);
    try {
      const [
        { data: scheduleCombos, error: schedErr },
      ...
      const stale = assignmentData.filter((a) => {
        if ((a.subject || "").toUpperCase() === "BP/BK") return false; // BK emang gak masuk kelas
        const uuid = codeToUuid.get(a.teacher_id);
        if (!uuid) return false; // kode gak ketemu di users -- jangan asal flag, kemungkinan data lain yang salah
        return !scheduleKeySet.has(`${uuid}|${a.class_id}`);
      });

      setStaleAssignments(stale);
    } catch (err) {
      console.error("Error checking stale assignments:", err);
      // Gagal cek -- diemin aja, jangan sampe nge-block halaman utama
      setStaleAssignments([]);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalMode === "edit") {
        ...
        // ✅ Ambil academic_year string dari academic_year_id yang dipilih
        const selectedAcademicYearData = academicYears.find(
          (ay) => ay.id === formData.academic_year_id,
        );

        if (!selectedAcademicYearData) {
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            academic_year: selectedAcademicYearData.year, // ✅ TAMBAH: String "2025/2026"
  ...
  const handleOpenDeleteModal = async (assignment) => {
    setDeleteTarget(assignment);
    setDeleteConfirmText("");
    setConfirmRiskAck(false);
    setJurnalCount(0);
    setShowDeleteModal(true);

    // Cek dulu apakah assignment ini udah dipake buat nulis jurnal harian.
    // Kalau ada, kasih warning ekstra sebelum admin bener-bener hapus --
    // soalnya jurnal_harian.assignment_id nunjuk ke teacher_assignments.id,
    // jadi kalau row ini dihapus histori jurnal yang udah ditulis guru bisa
    // jadi gak nyambung lagi.
    setCheckingJurnal(true);
    try {
      ...
      console.error("Error checking jurnal_harian:", error);
      // Gagal cek bukan alasan buat block user selamanya, tapi kita gak
      // tau juga aman apa nggak -- aman-nya anggap ADA histori (fail-safe)
      // biar admin tetep hati-hati & baca warning-nya.
      setJurnalCount(-1);
    } finally {
      setCheckingJurnal(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    // Kalau assignment ini punya histori jurnal harian (atau gagal dicek),
    // wajib centang checkbox "paham risiko" dulu sebelum bener-bener hapus.
    if (jurnalCount !== 0 && !confirmRiskAck) {
      showToast?.("Centang konfirmasi risiko jurnal harian dulu", "error");
      return;
    }
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      {/* ✅ NEW: Banner Stale Assignment -- assignment yang guru-nya udah
          gak kejadwal sama sekali di kelas itu (kemungkinan jadwal berubah
          di tengah semester tapi lupa update Penugasan Guru manual) */}
      {staleAssignments.length > 0 && (
        <div className="mb-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-800 rounded-xl overflow-hidden">
          <button
            onClick={() => setStaleBannerExpanded((prev) => !prev)}
            className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left">
            <div className="flex items-center gap-3 min-w-0">
              <AlertCircle className="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0" />
              <p className="text-sm font-semibold text-amber-800 dark:text-amber-300 truncate">
                {staleAssignments.length} penugasan perlu dicek -- guru gak
                kejadwal di kelas itu lagi
              </p>
            </div>
            {staleBannerExpanded ? (
          ...
          {staleBannerExpanded && (
            <div className="px-4 pb-4 space-y-2">
              <p className="text-xs text-amber-700 dark:text-amber-400 mb-2">
                Kemungkinan jadwal guru ini berubah di tengah semester (lewat
                Jadwal Massal atau Jadwal Mengajar), tapi penugasannya belum
                di-update manual di sini. Data presensi/nilai yang udah ada
                AMAN, tapi guru ini bisa jadi gak bisa input presensi/nilai baru
                buat kelas yang lama kalau dibiarkan.
              </p>
              <div className="space-y-1.5">
                {staleAssignments.map((a) => (
                  <div
                    key={a.id}
                    className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs bg-white dark:bg-gray-800 rounded-lg px-3 py-2 border border-amber-200 dark:border-amber-800">
                    <span className="font-semibold text-slate-700 dark:text-gray-200">
                      {a.users?.full_name || a.teacher_id}
                    </span>
                    <span className="text-slate-400 dark:text-gray-500">→</span>
                    <span className="text-slate-600 dark:text-gray-300">
                      Kelas {a.classes?.id || a.class_id}
                    </span>
                    <span className="px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-gray-700 text-slate-500 dark:text-gray-400">
                      {a.subject}
                    </span>
                  </div>
                ))}
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6">
              ...
              {/* ✅ TAMBAH: Cek & warning jurnal harian yang nempel ke assignment ini */}
              {checkingJurnal && (
                <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-6">
                  <Loader className="w-4 h-4 animate-spin" />
                  Mengecek histori jurnal harian...
                </div>
              )}

              {!checkingJurnal && jurnalCount !== 0 && (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm text-red-800 dark:text-red-300">
                      <p className="font-semibold mb-1">
                        ⚠️ Risiko Lebih Serius:
                      </p>
                      {jurnalCount > 0 ? (
                        <p className="text-xs">
                          Penugasan ini punya <b>{jurnalCount} jurnal harian</b>{" "}
                          yang udah ditulis guru. Kalau dihapus, histori jurnal
                          tersebut bisa jadi <b>gak nyambung lagi</b> ke
                          penugasan ini.
                        </p>
                      ) : (
                        <p className="text-xs">
                          Gagal ngecek histori jurnal harian buat penugasan ini.
                          Buat jaga-jaga, pastiin dulu penugasan ini emang belum
                          pernah dipake nulis jurnal sebelum dihapus.
                        </p>
                      )}
                    </div>
                  </div>
                  <label className="flex items-start gap-2 mt-3 text-xs text-red-800 dark:text-red-300 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={confirmRiskAck}
                      onChange={(e) => setConfirmRiskAck(e.target.checked)}
                      className="mt-0.5"
                    />
                    <span>
                      Saya paham risikonya dan tetap ingin menghapus penugasan
                      ini.
                    </span>
                  </label>
                </div>
