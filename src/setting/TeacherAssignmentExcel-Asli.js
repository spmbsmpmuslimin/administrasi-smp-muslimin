...
export const downloadTemplate = async (showToast) => {
  try {
    ...
    const instructionsData = [
      ...
      [
        "5. semester",
        "1 atau 2 (Saat ini semester aktif: " + activeAcademicInfo.activeSemester + ")",
      ],
      [""],
      ["TIPS:"],
...
export const validateImportData = async (jsonData, showToast) => {
  try {
    ...
    try {
      activeAcademicInfo = await getActiveAcademicInfo();
      warnings.push(`Semester aktif saat ini: ${activeAcademicInfo.activeSemester}`);
    } catch (error) {
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      } else if (activeAcademicInfo && row.semester != activeAcademicInfo.activeSemester) {
        warnings.push(
...
export const importAssignments = async (validData, mode = "skip", showToast) => {
  try {
    ...
    const message = `Import selesai: ${inserted} baru, ${updated} diupdate, ${skipped} di-skip ${
      activeAcademicInfo ? `(Semester aktif: ${activeAcademicInfo.activeSemester})` : ""
    }`;
...
export const generateErrorReport = async (validationResult) => {
  try {
    ...
    const summaryData = [
      ...
      ["Semester", activeAcademicInfo?.activeSemester || "Tidak Tersedia"],
      ["Status", activeAcademicInfo?.fullDisplayText || "Tidak Tersedia"],
    ];
