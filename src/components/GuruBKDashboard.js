...
import NotifikasiBk from "./NotifikasiBk";
import CatatanWalikelasBk from "./CatatanWalikelasBk";
...
const GuruBKDashboard = ({ user }) => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Get current academic year (dari service - sumber kebenaran tunggal,
      // bukan nebak dari tahun terbesar di tabel classes lagi)
      const activeInfo = await getActiveAcademicInfo();
      const currentYear = activeInfo?.year || "2025/2026";
  ...
  // Dipanggil pas guru BK klik notifikasi catatan siswa. Diarahin ke
  // halaman konseling sambil bawa student_id lewat navigation state --
  // sesuaikan lagi kalau halaman /konseling kamu butuh cara filter siswa
  // yang beda (misal query param atau route /konseling/:studentId).
  const handleOpenStudentFromNotif = (studentId) => {
    navigate("/konseling", { state: { studentId } });
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gray-900 p-3 sm:p-4 md:p-6">
      ...
      <div className="mb-6 sm:mb-8">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-slate-200 dark:border-gray-700 p-4 sm:p-6 lg:p-8">
          <div className="flex items-start justify-between gap-3">
            ...
            {/* Notifikasi catatan siswa berlabel "Perhatian" dari walikelas */}
            <NotifikasiBk currentUser={user} onOpenStudent={handleOpenStudentFromNotif} />
          </div>
        </div>
      </div>

      {/* Aksi Cepat (mobile & desktop) dihapus - shortcut (Presensi Guru, Konseling,
          Data Siswa, dll) sudah dipindah ke BottomNav mobile (role guru_bk). Lihat BottomNav.js. */}
      ...
      {/* Aksi Cepat desktop dihapus juga - lihat komentar di atas. */}
      ...
      {/* Catatan Dari Walikelas -- catatan "Perlu Perhatian" lengkap dari
          semua walikelas, isinya bukan cuma notifikasi ringkas */}
      <div className="mt-6 sm:mt-8">
        <CatatanWalikelasBk currentUser={user} />
      </div>
