// [file name]: components/Layout.js
...
const Layout = ({ user, onLogout, children, darkMode, onToggleDarkMode }) => {
  ...
  const getCurrentPageName = () => {
    const entry = menuByPath.get(location.pathname);
    if (!entry?.title) return "Dashboard";
    // title bisa string statis, atau function (user) => string buat
    // halaman yang label-nya beda tergantung role (contoh: attendance-teacher).
    return typeof entry.title === "function" ? entry.title(user) : entry.title;
  };
