...
export const Login = ({ onLogin, onShowToast }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1️⃣ Cek dulu ke tabel `users` (guru, admin, tu, guru_bk, petugas_perpus)
      ...
      // ✅ Ketemu di tabel `users` -> proses login staf/guru seperti biasa
      if (data) {
        ...
        // ✅ Catat device yang dipakai buat login ini (buat tab
        // "Perangkat Terhubung" di Setting -> Active Users). Fungsi ini
        // sengaja gak pernah nge-throw, jadi kalaupun gagal, gak bakal
      ...
      // ❌ Nggak ketemu di `users` -> ini bukan akun staf
      throw new Error(
          ...
          <div className="bg-slate-800/70 backdrop-blur-2xl border border-white/20 rounded-3xl p-6 sm:p-7 shadow-2xl shadow-blue-500/10">
            ...
            <div className="mt-5 pt-4 border-t border-slate-700/50 text-center">
              <button
                ...
                className="text-base font-bold text-blue-300 hover:text-blue-200 transition-colors mb-2">
                Login Sebagai Siswa 👉
              </button>
