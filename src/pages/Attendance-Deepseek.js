...
const Attendance = ({ user, onShowToast }) => {
  ...
  const handleExportExcel = async (filters) => {
    try {
      ...
      // Query data berdasarkan filters dari ExportModal
      let query = supabase
        .from('attendances')
        .select(`
          *,
      ...
      // Filter by subject jika bukan presensi harian
      if (!isHomeroomDaily()) {
      ...
      if (!data || data.length === 0) {
        if (onShowToast) {
          onShowToast("Tidak ada data presensi untuk diekspor dalam rentang tanggal yang dipilih", "warning");
        }
        return;
      }
      ...
      const formattedData = data.map(record => ({
        ...
        nama: record.students.full_name,
        kelas: record.classes.id,
        status: record.status,
        keterangan: record.notes || '',
        mata_pelajaran: record.subject,
        tipe: record.type
      }));
