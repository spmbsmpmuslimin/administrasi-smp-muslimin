                ...
                {siswaWithNotes.length === 0 ? (
                  <tr>
                    <td
                      ...
                      Catatan Siswa untuk kelas ini belum ada, silahkan
                      tambahkan.
                    </td>
                  </tr>
                ) : filteredSiswa.length === 0 ? (
...
const CatatanSiswa = ({ user, onShowToast }) => {
  ...
  // Kirim notifikasi ke semua guru BK/BP (role = 'guru_bk') pas walikelas
  // bikin catatan baru berlabel "perhatian". Sengaja di-wrap try/catch
  // sendiri dan gak nge-throw ke pemanggilnya -- kalau notifikasi gagal
  // terkirim, itu gak boleh sampai bikin catatan yang UDAH kesimpen jadi
  // keliatan gagal di mata walikelas. Notifikasi itu "bonus", bukan
  // syarat catatan berhasil disimpan.
  const notifyGuruBk = async ({
    noteId,
    studentId,
    studentName,
    classId,
    ...
    try {
      const { data: bkTeachers, error: bkErr } = await supabase
        .from("users")
        .select("id")
        .eq("role", "guru_bk")
        .eq("is_active", true);
      ...
      if (!bkTeachers || bkTeachers.length === 0) {
        console.log("ℹ️ Gak ada guru BK aktif terdaftar, notifikasi dilewati");
        return;
      }

      const notifRows = bkTeachers.map((bk) => ({
        recipient_id: bk.id,
        type: "catatan_perhatian",
        title: "Catatan Perhatian Baru",
        message: `${teacherName} menambahkan catatan berlabel "Perlu Perhatian" untuk ${studentName} (Kelas ${classId}).`,
        // Isi catatan lengkap disalin langsung ke sini (denormalisasi) --
        // lihat catatan desain di notifications_migration.sql kenapa.
      ...
      console.log(`🔔 Notifikasi terkirim ke ${notifRows.length} guru BK`);
    } catch (err) {
      // Sengaja cuma di-log, gak ditampilin ke walikelas -- lihat catatan
      // di atas kenapa error di sini gak boleh ganggu alur simpan catatan.
      console.error("⚠️ Gagal kirim notifikasi ke guru BK:", err);
    }
  };
  ...
  const handleCreateNote = async (e) => {
    ...
    try {
      ...
      // .select() dengan join ke students biar dapet nama siswa buat isi
      // pesan notifikasi tanpa perlu round-trip query terpisah.
      ...
      // Notifikasi ke guru BK/BP cuma dipicu buat catatan berlabel
      // "perhatian" -- lihat catatan di notifyGuruBk kenapa cuma label ini.
      if (formData.label === "perhatian") {
        await notifyGuruBk({
          noteId: inserted.id,
          studentId: formData.student_id,
          studentName: inserted.student?.full_name || "siswa",
          classId: currentClass,
          teacherName: currentUser.full_name,
          category: formData.category,
          label: formData.label,
          noteContent: formData.note_content,
  ...
  // Siswa yang beneran udah punya catatan (positif/perhatian/netral > 0).
  // Siswa yang belum pernah dicatat gak usah nongol di dashboard/tabel --
  // baru muncul kalau gurunya bikin catatan pertama buat siswa itu.
  const siswaWithNotes = useMemo(
    () =>
      siswaList.filter(
        (s) => (s.positif || 0) + (s.perhatian || 0) + (s.netral || 0) > 0,
      ),
    [siswaList],
  );
  ...
  const filteredSiswa = useMemo(
    () =>
      siswaWithNotes.filter(
        (s) =>
          s.nama.toLowerCase().includes(searchTerm.toLowerCase()) ||
          s.nis.includes(searchTerm),
      ),
    [siswaWithNotes, searchTerm],
  );
