...
const AttendanceManagement = ({ user, onShowToast }) => {
  ...
  // Check auth - PERBAIKAN: Ambil teacher_id langsung dari user prop
  useEffect(() => {
    const checkAuth = async () => {
      try {
        if (user) {
          ...
          // Coba ambil teacher_id dari user prop langsung
          if (user.teacher_id) {
            setTeacherId(user.teacher_id);
            console.log("✅ Teacher ID dari user prop:", user.teacher_id);
          } else {
            // Fallback: query dari database
