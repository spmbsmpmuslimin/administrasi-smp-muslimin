  ...
  const fetchClasses = async () => {
    try {
      ...
      if (!assignments || assignments.length === 0) {
        console.warn("⚠️ Guru belum memiliki penugasan kelas, menampilkan semua kelas");
        ...
        setClasses(allClasses || []);
        console.log(`🔄 FALLBACK: Menampilkan ${allClasses?.length || 0} kelas`);
        return;
      }
      ...
      console.log(`✅ Guru mengampu ${classesData?.length || 0} kelas:`, classesData);
    } catch (err) {
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border-2 border-indigo-200 dark:border-indigo-800 p-4 mb-8">
            ...
            <div
              className={`grid gap-2 ${
                ...
                getGroupedTodaySchedules().length > 4 && getGroupedTodaySchedules().length <= 6
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-600 dark:text-gray-400 truncate">
                          ...
                          {group.length > 1 &&
                            ` (${
                              findPeriodsByTimeRange(first.day, first.start_time, first.end_time)[0]
                      ...
                      <div className="flex flex-shrink-0 gap-1 ml-2">
                        ...
                        <button
                          onClick={() => {
                            if (window.confirm(`Hapus sesi Kelas ${first.class_id}?`)) {
                              group.forEach((schedule) => handleDelete(schedule.id));
                            }
                          }}
