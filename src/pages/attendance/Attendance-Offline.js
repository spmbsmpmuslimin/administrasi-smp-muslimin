...
const Attendance = ({ user, onShowToast }) => {
  ...
  useEffect(() => {
    ...
    offlineHelper.subscribe((event) => {
      if (event.type === "online") {
        ...
        if (onShowToast) onShowToast("✅ Sync selesai!", "success");
        updatePendingCount();
      }
    });
  ...
  const processAttendanceSubmission = async () => {
    ...
    try {
        ...
        await offlineHelper.addPending({
          ...
          syncFn: async (data) => {
            ...
            for (let i = 0; i < data.length; i += BATCH_SIZE) {
              ...
              await supabase.from("attendances").insert(batch);
            }
          },
        });
      ...
      // ✅ FALLBACK: Save offline on error
      try {
        ...
        <div className="fixed bottom-4 right-4 z-50">
          ...
          >
            <div className="flex items-center gap-2 text-sm font-semibold">
              ...
              {pendingCount > 0 && (
                <span className="ml-2 bg-white/30 px-2 py-1 rounded-md text-xs">
