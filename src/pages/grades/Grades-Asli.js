  ...
  useEffect(() => {
    const fetchAcademicYears = async () => {
      ...
      try {
        ...
        setMessage("Error: Terjadi kesalahan sistem saat mengambil tahun akademik");
      }
    };
    ...
    const fetchSubjects = async () => {
      ...
      try {
        ...
        if (!data || data.length === 0) {
          setSubjects([]);
          setMessage(`Tidak ada mata pelajaran untuk ${academicYear} Semester ${semester}`);
          return;
        }
    ...
    const fetchClasses = async () => {
      ...
      try {
        ...
        if (!assignmentData?.length) {
          setClasses([]);
          setMessage(`Tidak ada kelas untuk ${selectedSubject} Semester ${semester}`);
          return;
        }
              ...
              {activeAcademicInfo && (
                <div className="flex flex-wrap items-center gap-2 mt-2">
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-lg text-xs">
                    ...
                    <span className="font-bold">{activeAcademicInfo.displayText}</span>
                  </div>
            ...
            <div>
              ...
              >
                ...
                {academicYears.map((year) => (
                  <option key={year} value={year} className="dark:bg-gray-700 dark:text-slate-100">
                    {year}
                  </option>
                ))}
              </select>
        ...
        {selectedClass && selectedSubject && students.length === 0 && !loading && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm dark:shadow-gray-900/30 border border-slate-200 dark:border-gray-700 p-6 sm:p-8 md:p-12 text-center">
            ...
            <p className="text-sm sm:text-base text-slate-400 dark:text-slate-500">
              Tidak ada siswa aktif di kelas ini untuk tahun akademik {academicYear} Semester{" "}
              {semester}
            </p>
          </div>
        )}
