// 📁 GradesKatrol.js - VERSI FLEXIBLE NO LOCK (REVISED) dengan Academic Service - MULTI SEMESTER SUPPORT
...
// ✅ UPDATE IMPORT ACADEMIC YEAR SERVICE - TAMBAH FUNGSI MULTI SEMESTER
import {
...
const GradesKatrol = ({
  ...
  // ✅ UPDATE STATE UNTUK MULTI SEMESTER SUPPORT
  // ❌ HAPUS STATE LAMA
  ...
  // ✅ TAMBAH academicInfo tetap untuk compatibility (akan diisi dari selectedSemesterInfo)
  ...
  // State untuk filter - ✅ DIUBAH: Gunakan nilai dari selectedSemesterInfo
  ...
  // ✅ LOAD ACTIVE ACADEMIC INFO SAAT COMPONENT MOUNT
  useEffect(() => {
    const loadActiveAcademicInfo = async () => {
      try {
        ...
        // Untuk compatibility, set juga academicInfo
        setAcademicInfo(info);
      } catch (error) {
        console.error("❌ Error loading active academic info:", error);
        setMessage({
          text: "Gagal memuat informasi akademik aktif. Mohon refresh halaman.",
          type: "error",
        });
      } finally {
  ...
  useEffect(() => {
    const loadAvailableSemesters = async () => {
      ...
      try {
        ...
        // Default ke semester aktif jika ada props, jika tidak ke semester aktif system
        let defaultSemesterId = null;

        if (semester && academicYear) {
          // Cari semester berdasarkan props lama
          const matchedSem = semesters.find(
            (s) => s.semester === parseInt(semester) && s.year === academicYear
          );
          if (matchedSem) {
            defaultSemesterId = matchedSem.id;
          }
        }

        // Jika tidak ada dari props, ambil semester aktif
        if (!defaultSemesterId) {
          const activeSem = semesters.find((s) => s.is_active);
          if (activeSem) {
            defaultSemesterId = activeSem.id;
          }
        }

        // Fallback ke semester pertama
        ...
        if (defaultSemesterId) {
          ...
          const semInfo = semesters.find((s) => s.id === defaultSemesterId);
          setSelectedSemesterInfo(semInfo);
        }
      } catch (error) {
        console.error("❌ Error loading available semesters:", error);
        setMessage({
          text: "Gagal memuat daftar semester yang tersedia",
          type: "error",
        });
      }
    };
  ...
  const handleSemesterChange = async (semesterId) => {
    console.log("🔄 Semester change to:", semesterId);

    if (!semesterId) {
      setSelectedSemesterId(null);
      setSelectedSemesterInfo(null);
      return;
    }
    ...
    // Untuk compatibility, update juga academicInfo
    if (semInfo) {
      setAcademicInfo({
        ...semInfo,
        displayText: `${semInfo.year} - Semester ${semInfo.semester}${
          !semInfo.is_active ? " (Arsip)" : ""
        }`,
        semesterText: `Semester ${semInfo.semester}`,
        yearId: semInfo.id,
      });
    }

    showMessage(`Semester berubah ke: ${semInfo?.year} - Semester ${semInfo?.semester}`, "info");
  };
  ...
  // ✅ UPDATE: Fetch Academic Years - GUNAKAN selectedSemesterInfo
  useEffect(() => {
    console.log("🔄 fetchAcademicYears TRIGGERED", {
      teacherId,
      selectedSemesterInfo,
    });
    const fetchAcademicYears = async () => {
      if (!teacherId || !selectedSemesterInfo) {
        console.log("⚠️ teacherId or semester info not available, skipping fetch");
        setIsInitialLoad(false);
        return;
      }
      console.log("📡 Fetching academic years...");
      try {
        // ✅ UPDATE: Filter berdasarkan academic_year_id
        ...
        if (!assignmentData || assignmentData.length === 0) {
          setAcademicYears([]);
          setMessage({
            text: "Tidak ada data tahun akademik untuk guru ini pada semester ini",
            type: "error",
          });
          setIsInitialLoad(false);
          return;
    ...
    fetchAcademicYears();
  }, [teacherId, selectedSemesterId, selectedSemesterInfo]);

  // ✅ UPDATE: Fetch Subjects - GUNAKAN selectedSemesterId
  useEffect(() => {
    ...
    const fetchSubjects = async () => {
      ...
      try {
        // ✅ UPDATE: Gunakan academic_year_id sebagai filter utama
        ...
        if (!data || data.length === 0) {
          ...
          if (selectedSemesterInfo) {
            setMessage({
              text: `Tidak ada mata pelajaran untuk ${selectedSemesterInfo.year} semester ${selectedSemesterInfo.semester}`,
              type: "error",
            });
          } else {
            setMessage({
              text: `Tidak ada mata pelajaran untuk semester ini`,
              type: "error",
            });
          }
          return;
  ...
  // ✅ UPDATE: Fetch Classes - GUNAKAN selectedSemesterId
  useEffect(() => {
    ...
    const fetchClasses = async () => {
      if (!selectedSubjectState || !teacherId || !selectedSemesterId) {
      ...
      try {
        ...
        if (!assignmentData?.length) {
          ...
          setClasses([]);
          setMessage({
            text: "Tidak ada kelas untuk mata pelajaran ini pada semester ini",
            type: "error",
          });
          return;
        }
        ...
        const { data: classData, error: classError } = await supabase
          ...
          .select("id, grade")
          .in("id", classIds)
          .eq("academic_year_id", selectedSemesterId); // ✅ UPDATE: Filter dengan academic_year_id
    ...
    fetchClasses();
  }, [selectedSubjectState, teacherId, selectedSemesterId, selectedSemesterInfo]);

  // ✅ UPDATE: LOAD KKM SETTINGS - GUNAKAN selectedSemesterId
  const loadKkmSettings = async () => {
    if (!selectedSubjectState || !selectedClassId || !selectedSemesterId) {
      setKkmSettings(null);
      return;
    }
  ...
  // ✅ UPDATE: TAMBAH useEffect INI (GUNAKAN selectedSemesterId)
  useEffect(() => {
    if (selectedSubjectState && selectedClassId && selectedSemesterId) {
      loadKkmSettings();
    } else {
  ...
  }, [selectedSubjectState, selectedClassId, selectedSemesterId, selectedSemesterInfo]);

  // ✅ UPDATE: SAVE/UPDATE KKM SETTINGS - GUNAKAN selectedSemesterId
  const saveKkmSettings = async () => {
    // Validasi
    if (!selectedSubjectState || !selectedClassId || !selectedSemesterId) {
      showMessage("Pilih kelas, mata pelajaran, dan semester terlebih dahulu!", "error");
      return;
    }
    ...
    try {
      const settingsData = {
        ...
        academic_year_id: selectedSemesterId, // ✅ UPDATE: Simpan academic_year_id
        updated_at: new Date().toISOString(),
        updated_by: user?.id || teacherId,
      };

      // ✅ TAMBAH ACADEMIC YEAR INFO dari selectedSemesterInfo
      if (selectedSemesterInfo) {
        settingsData.academic_year = selectedSemesterInfo.year;
        settingsData.semester = selectedSemesterInfo.semester;
      }
        ...
        try {
          const { data, error: upsertError } = await supabase
            ...
            .upsert({
              ...
              kkm: kkm,
              nilai_maksimal: nilaiMaksimal,
              academic_year_id: selectedSemesterId, // ✅ UPDATE
              updated_at: new Date().toISOString(),
              updated_by: user?.id || teacherId,
              ...(selectedSemesterInfo && {
  ...
  // ✅ UPDATE: FETCH DATA NILAI - GUNAKAN selectedSemesterId
  const fetchDataNilai = async () => {
    if (!selectedClassId || !selectedSubjectState || !selectedSemesterId) {
      showMessage("Pilih kelas, mata pelajaran, dan semester terlebih dahulu", "error");
      return;
    }
    ...
    try {
      ...
      // ✅ UPDATE: Filter dengan academic_year_id
      ...
      if (katrolData && katrolData.length > 0) {
        ...
        // ✅ Ambil data siswa untuk mapping nama & NIS secara terpisah
        ...
        // ✅ Format data dari database dengan kolom yang BENAR
      ...
      // ✅ UPDATE: Filter students dengan academic_year_id
      ...
      // ✅ UPDATE: Filter grades dengan academic_year_id
  ...
  // ✅ UPDATE: PROSES KATROL - GUNAKAN selectedSemesterId
  const prosesKatrol = async () => {
    ...
    setProcessing(true);
    try {
      // ✅ AMBIL DATA SISWA DENGAN SORTING - Filter dengan academic_year_id
      const { data: studentsData, error: studentsError } = await supabase
        .from("students")
        .select("id, full_name, nis")
        .eq("class_id", selectedClassId)
      ...
      // Ambil data grades dengan academic_year_id
      const { data: gradesData, error: gradesError } = await supabase
        .from("grades")
        .select("*")
        .eq("class_id", selectedClassId)
      ...
      const additionalInfo = {
        ...
        semester: selectedSemesterInfo?.semester || null,
        academic_year: selectedSemesterInfo?.year || null,
      };
  ...
  // ✅ UPDATE: SIMPAN KE DATABASE - GUNAKAN selectedSemesterId
  const saveKatrolToDatabase = async () => {
    ...
    // ✅ GUNAKAN DATA DARI SELECTED SEMESTER INFO UNTUK DISPLAY
    const semesterDisplay = selectedSemesterInfo
      ? `Semester ${selectedSemesterInfo.semester}`
      : "Semester";
    const yearDisplay = selectedSemesterInfo?.year || "";
  ...
  const handleFirstConfirm = async () => {
    ...
    try {
      // Cek data lama - UPDATE: Filter dengan academic_year_id
  ...
  // ✅ UPDATE: processSaveToDatabase - GUNAKAN selectedSemesterId
  const processSaveToDatabase = async () => {
    setSaving(true);

    try {
      // Hapus data lama jika ada - UPDATE: Filter dengan academic_year_id
      ...
      const recordsToSave = hasilKatrol.map((item) => {
        ...
        const katrolData = {
          ...
          semester: selectedSemesterInfo?.semester || null,
          academic_year: selectedSemesterInfo?.year || null,
          // ✅ hasilKatrol sudah dalam format display (nh1, nh1_k)
          nh1_mentah: item.nh1,
          nh2_mentah: item.nh2,
          nh3_mentah: item.nh3,
      ...
      const successMessage = selectedSemesterInfo
        ? `✅ Berhasil menyimpan ${recordsToSave.length} nilai katrol ke database untuk ${selectedSemesterInfo.year} - Semester ${selectedSemesterInfo.semester}!`
  ...
  // ✅ UPDATE: EXPORT EXCEL - GUNAKAN selectedSemesterInfo
  const handleExport = async () => {
    ...
    try {
      ...
      // ✅ TAMBAH INFO SEMESTER DI METADATA
      const exportMetadata = {
        subject: selectedSubjectState,
        class_name: selectedClassId,
        academic_year: selectedSemesterInfo?.year || "",
        semester: selectedSemesterInfo?.semester || "",
        semester_display: selectedSemesterInfo
          ? `Semester ${selectedSemesterInfo.semester}`
          : "Semester",
        kkm: kkm,
        target_max: nilaiMaksimal,
        ...
        academic_info: selectedSemesterInfo
          ? `${selectedSemesterInfo.year} - Semester ${selectedSemesterInfo.semester}${
              !selectedSemesterInfo.is_active ? " (Arsip)" : ""
            }`
          : "",
        is_archive_mode: selectedSemesterInfo ? !selectedSemesterInfo.is_active : false,
      };

      exportToExcel(
        hasilKatrol,
        exportMetadata,
        `Katrol_${selectedSubjectState}_${selectedClassId}_${
          selectedSemesterInfo?.semester || ""
        }_${(selectedSemesterInfo?.year || "").replace("/", "-")}`
      );
  ...
  // ✅ UPDATE: LOADING STATE UNTUK ACADEMIC INFO
  if (loadingAcademicInfo) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <Loader className="w-12 h-12 text-purple-600 animate-spin mx-auto mb-4" />
          <p className="text-gray-600 dark:text-gray-400">Memuat informasi akademik...</p>
        </div>
      </div>
    );
  }

  // ✅ UPDATE: JIKA TIDAK ADA SEMESTER YANG TERSEDIA
  if (availableSemesters.length === 0 && !loadingAcademicInfo) {
    return (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-red-700 dark:text-red-300 mt-1">
                  Tidak dapat menemukan semester yang bisa diakses. Pastikan Anda memiliki akses ke
      ...
      <div className="max-w-7xl mx-auto mb-6">
        <div className="flex items-center justify-between bg-gradient-to-r from-purple-600 to-indigo-700 dark:from-purple-950 dark:to-indigo-900 rounded-xl shadow-lg p-4 sm:p-6 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl md:text-3xl font-bold">Katrol Nilai SMP</h1>
              {/* ✅ UPDATE: TAMPILKAN INFO SEMESTER DI HEADER */}
              <p className="text-sm text-white/80 mt-1">
                {selectedSemesterInfo ? (
                  <span>
                    Linear Scaling Method untuk {selectedSemesterInfo.year} - Semester{" "}
                    {selectedSemesterInfo.semester}
                    {!selectedSemesterInfo.is_active && " (Mode Arsip)"}
                  </span>
                ) : (
      ...
      <div className="max-w-7xl mx-auto mb-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4 sm:p-6 border border-gray-100 dark:border-gray-700">
          <h2 className="text-lg font-semibold mb-4 dark:text-gray-200 flex items-center gap-2">
            ...
            Filter Data
            {/* ✅ UPDATE: BADGE INFORMASI SEMESTER */}
            {selectedSemesterInfo && (
              <span
                className={`ml-auto px-3 py-1 rounded-full text-xs ${
                  selectedSemesterInfo.is_active
                    ? "bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300"
                    : "bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-300"
                }`}
              >
                {selectedSemesterInfo.is_active ? "✅ Aktif" : "📁 Arsip"}
              </span>
            )}
          </h2>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            {/* ✅ UPDATE: Dropdown Semester Dinamis */}
            <div className="lg:col-span-2">
              ...
              <select
                ...
                disabled={isInitialLoad || loading || availableSemesters.length === 0}
                style={{
                  position: "relative",
                  zIndex: 9999,
                  pointerEvents: "auto",
                  cursor:
                    isInitialLoad || loading || availableSemesters.length === 0
                      ? "not-allowed"
                      : "pointer",
                }}
              >
                ...
                {availableSemesters.map((sem) => (
                  ...
                  >
                    ...
                    {sem.is_active ? " ✅ (Aktif)" : " 📁 (Arsip)"}
                  </option>
                ))}
              </select>

              {/* ✅ INDIKATOR MODE ARSIP */}
              {selectedSemesterInfo && !selectedSemesterInfo.is_active && (
                <div className="mt-2 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-2">
                  <div className="flex items-center gap-2 text-xs text-orange-700 dark:text-orange-300">
                    ...
                    <span>
                      <strong>Mode Arsip:</strong> Data dari semester yang tidak aktif
                    </span>
                  </div>
                </div>
              )}
            ...
            <div>
              ...
              <select
                key={`subject-${subjects.length}-${selectedSemesterId}`}
                ...
                className="w-full px-4 py-3 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white dark:bg-gray-700 dark:text-gray-200 transition-colors"
                disabled={isInitialLoad || loading || !selectedSemesterId}
                style={{
                  position: "relative",
                  zIndex: 9999,
                  pointerEvents: "auto",
                  cursor:
                    isInitialLoad || loading || !selectedSemesterId ? "not-allowed" : "pointer",
                }}
              >
            ...
            <div>
              ...
              <select
                key={`class-${classes.length}-${selectedSubjectState}-${selectedSemesterId}`}
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold dark:text-gray-200 flex items-center gap-2">
                ...
                Pengaturan KKM
                {/* ✅ UPDATE: INFO SEMESTER CONTEXT */}
                {selectedSemesterInfo && (
                  <span className="text-sm font-normal text-gray-500 dark:text-gray-400 ml-2">
                    untuk {selectedSemesterInfo.year} - Semester {selectedSemesterInfo.semester}
                  </span>
                )}
              </h3>
            ...
            {/* Grid 3 kolom untuk KKM + Nilai Maksimal + Tombol */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
              ...
              <div className="flex items-end">
                {selectedClassId && selectedSubjectState && selectedSemesterId && (
                  <button
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              disabled={!selectedClassId || !selectedSubjectState || !selectedSemesterId || loading}
            ...
            <button
              ...
              disabled={
                !selectedClassId || !selectedSubjectState || !selectedSemesterId || processing
              }
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 border border-gray-100 dark:border-gray-700">
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-4 text-sm">
                ...
                {/* ✅ UPDATE: TAMPILKAN INFO SEMESTER */}
                {selectedSemesterInfo && (
                  <div
                ...
                * Nilai yang dikatrol ditandai dengan warna hijau
                {selectedSemesterInfo && (
                  <span className="block mt-1">
                    Data untuk {selectedSemesterInfo.year} - Semester{" "}
                    {selectedSemesterInfo.semester}
                    {!selectedSemesterInfo.is_active && " (Mode Arsip)"}
                  </span>
                )}
