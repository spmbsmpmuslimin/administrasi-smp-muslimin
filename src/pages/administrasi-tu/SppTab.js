...
// ============================================================
// Label pendek buat header kolom bulan (Jul, Agu, ... Jun) -- beda dari
// MONTH_NAMES yang lengkap (dipake di tempat laen kayak Riwayat).
const MONTH_SHORT = [
  "Jan",
  "Feb",
  "Mar",
...
// Icon status kecil per bulan -- centang & silang POLOS (bukan yang
  ...
  return (
    <div className="space-y-6">
      ...
      {rows.length > 0 && (
        ...
        >
          <span>Klik baris siswa buat langsung catat pembayarannya.</span>
          <span className="flex items-center gap-1.5">
            <Check size={16} strokeWidth={3} className="text-emerald-500" /> Lunas
          </span>
          <span className="flex items-center gap-1.5">
            <Circle size={14} className="text-amber-500 fill-amber-500" /> Sebagian
          </span>
          <span className="flex items-center gap-1.5">
          ...
          >
            <tr>
              ...
              {headerPeriods.map((p) => (
                <th
                  key={p.key}
                  className="px-2 py-3 text-center text-xs font-semibold tracking-wide whitespace-nowrap"
                >
