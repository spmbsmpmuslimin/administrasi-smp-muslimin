...
const ExportExcel = ({ attendances, teachers, month, year, monthName, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-theme-bg rounded-xl shadow-2xl max-w-md w-full p-6">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            <FileSpreadsheet className="text-green-600" size={24} />
            <h3 className="text-xl font-bold text-theme">Export ke Excel</h3>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-theme-surface rounded-lg transition-all">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 px-4 py-3 bg-gray-200 hover:bg-gray-300 text-theme-secondary font-semibold rounded-lg transition-all"
          >
            Batal
          </button>
          <button
