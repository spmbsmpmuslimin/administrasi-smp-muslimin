...
function StatusBadge({ status }) {
  const styles = {
    Lengkap: "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 border-green-300 dark:border-green-800",
    Sebagian: "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 border-yellow-300 dark:border-yellow-800",
    "Belum Dicek": "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-300 dark:border-gray-700",
    "Butuh DB Manual": "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 border-red-300 dark:border-red-800",
  };
  return (
...
export default function CoverageChecker() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((r) => (
              ...
              >
                <td className="p-3 font-mono text-xs text-gray-800 dark:text-gray-200">{r.table}</td>
                <td className="p-3 text-xs text-gray-600 dark:text-gray-400">
                  {r.page || <span className="text-red-500 dark:text-red-400 italic">belum ada</span>}
                </td>
                <td className="p-3 text-center"><CrudDot value={r.create} /></td>
                <td className="p-3 text-center"><CrudDot value={r.read} /></td>
                <td className="p-3 text-center"><CrudDot value={r.update} /></td>
                <td className="p-3 text-center"><CrudDot value={r.delete} /></td>
      ...
      {filtered.length === 0 && (
        <p className="text-center text-gray-400 dark:text-gray-600 py-8">Tidak ada tabel yang cocok.</p>
      )}
    </div>
  );
}
