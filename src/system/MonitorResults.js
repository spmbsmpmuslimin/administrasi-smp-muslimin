...
function MonitorResults({ results, isChecking }) {
  ...
  const renderCheckItem = (check, index) => {
    return (
      <div key={index} className="py-4 first:pt-3 border-b last:border-b-0 border-gray-100">
        <div className="flex items-start gap-3">
          <span className="text-xl mt-0.5 flex-shrink-0">{getStatusIcon(check.severity)}</span>
          
          <div className="flex-1 min-w-0">
            <h4 className="font-medium text-gray-900 mb-1">{check.title || check.message}</h4>
            ...
            {check.details && (
              <div className="mt-2 space-y-1">
                {check.details.description && (
                  <p className="text-sm text-gray-600">{check.details.description}</p>
                )}
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                  ...
                  {(check.details.affectedRecords !== undefined || check.details.count !== undefined) && (
                    <div className="flex items-center gap-1.5">
                      <span>📊</span>
          ...
          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border flex-shrink-0 ${getBadgeClass(check.severity)}`}>
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${getBadgeClass(categoryData.status)}`}>
          ...
          <ul className="space-y-1 text-sm text-red-700">
            {errors.map((err, idx) => (
              <li key={idx} className="flex items-start gap-2">
                ...
                <span><strong>{err.checker}:</strong> {err.error}</span>
              </li>
            ))}
          </ul>
        </div>
