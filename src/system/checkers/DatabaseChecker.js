...
const checkConnection = async () => {
  try {
    const { data, error } = await supabase.from("academic_years").select("id").limit(1);
...
const checkTablesExist = async () => {
  ...
  try {
    ...
    const checks = await Promise.allSettled(
      CRITICAL_TABLES.map((tableName) => supabase.from(tableName).select("id").limit(1))
    );
...
const checkOrphanedRecords = async () => {
  ...
  try {
    ...
    const [studentsCheck, attendancesCheck, gradesCheck, assignmentsCheck] = await Promise.all([
...
const checkOrphanedGrades = async () => {
  ...
  try {
    const { data: grades } = await supabase.from("grades").select("student_id").limit(200);
