...
const MaintenanceModeTab = ({ showToast }) => {
  ...
  // 🔥 Load users TANPA developer (developer selalu bypass maintenance,
  // admin sekarang IKUT butuh whitelist manual kalau mau akses pas maintenance)
  const loadAllUsers = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq("is_active", true)
        .neq("role", "developer") // 🔥 Filter developer
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      // Pas diaktifin, sekalian simpan pesan yang lagi tampil di form
      // (entah itu dari DB atau masih fallback default) supaya row
      // "maintenance_message" dijamin selalu ada isinya, ga nunggu admin
      // ngetik dulu di textarea.
      const upsertPayload = [
        {
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
          <div className="flex-1">
            ...
            <p className="text-xs sm:text-sm text-blue-600 dark:text-gray-400 mt-1">
              Ketika diaktifkan, hanya developer + user di whitelist yang bisa akses
            </p>
          </div>
          <button
